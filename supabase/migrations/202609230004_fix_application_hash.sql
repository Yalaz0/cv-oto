...
begin
  ...
  values(v_owner,nullif(btrim(p_company_name),''),nullif(btrim(p_job_title),''),p_job_description,encode(digest(convert_to(p_job_description,'UTF8'),'sha256'),'hex'),p_document_locale,v_version)
